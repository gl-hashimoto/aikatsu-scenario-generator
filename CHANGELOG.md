...
### 新機能
- ✨ **完了したジョブの自動保存機能**
  - 分析が完了すると自動的に履歴に保存されるように変更
  - 「💾 履歴に保存」ボタンを削除し、ワンクリック不要に
  - 完了したジョブには「✅ 自動的に履歴に保存されました」と表示
...
### 改善
- 🎨 **UI/UXの全体最適化**
  - タブを削除してシンプルな縦スクロールレイアウトに変更
  - 「📝 新規分析」「📚 保存済記事ネタ提案」のタブ構造を廃止
  - すべてのコンテンツを1ページに統合
  - ステップがないため、よりシンプルで直感的なUIに
...
### 重要な変更
- 🎯 **アプリの名称変更**
  - 「漫画シナリオ生成AI - 愛カツ専用版」→「記事ネタ提案ツール」
  - アプリアイコンを 📖 → 💡 に変更
  - 記事ネタ提案機能に特化したシンプルな名称に

### 改善
- 📝 **メニューの簡素化**
  - 「📝 ネタ管理」を非表示（現在未使用）
  - 「🤖 シナリオ生成」を非表示（現在未使用）
  - メニューは「💡 記事ネタ提案」と「⚙️ 設定」のみに
...
### 改善
- 📝 **UI文言の変更**
  - 「記事分析＆ネタ展開」→「記事ネタ提案」に変更
  - 「保存済みの分析」→「保存済記事ネタ提案」に変更
  - メニューアイコンを 🔬 → 💡 に変更

### バグ修正
- 🐛 **タイトル自動生成機能**
  - タイトルが空の場合、記事内容の最初の50文字を自動的にタイトルとして使用
  - 「（タイトルなし）」が表示される問題を解決
  - 改行やタブを除去して見やすく整形
